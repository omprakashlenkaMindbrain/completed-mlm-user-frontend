...
export default function ProfilePage() {
  ...
  const handleOpenReferralTree = () => {
    // Build tree structure from profile referral data
    if (profile?.referrals) {
      const treeData = buildReferralTree(profile.referrals, profile.id)
      setReferralTreeData(treeData)
    }
    setIsReferralModalOpen(true)
  }

  const buildReferralTree = (referrals, userId) => {
    ...
    referrals.forEach((ref) => {
      if (!nodeMap[ref.id]) {
        nodeMap[ref.id] = {
          ...
          leftLeg: null,
          rightLeg: null,
          position: ref.position || "none",
        }
      }
    })

    // Add the root (logged in user)
    const root = {
      id: userId,
    ...
    // Build tree structure based on positions (left/right legs)
    referrals.forEach((ref) => {
      if (ref.parentId === userId) {
        if (ref.position === "left") {
          root.leftLeg = nodeMap[ref.id]
        } else if (ref.position === "right") {
          root.rightLeg = nodeMap[ref.id]
        }
      } else {
        const parent = nodeMap[ref.parentId]
        if (parent) {
          if (ref.position === "left") {
            parent.leftLeg = nodeMap[ref.id]
          } else if (ref.position === "right") {
            parent.rightLeg = nodeMap[ref.id]
          }
        }
      }
    })
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div
            className="w-12 h-12 rounded-full border-4 border-slate-300 border-t-blue-800 animate-spin mx-auto mb-4"
            style={{ borderTopColor: PRIMARY_NAVY }}
          ></div>
  ...
  if (error) {
    return <div className="min-h-screen flex items-center justify-center text-red-600 font-medium">{error}</div>
  }
        ...
        <div className="relative mb-8 rounded-2xl overflow-hidden shadow-xl bg-white">
          ...
          <div className="px-6 sm:px-8 pb-6">
            <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-16 relative z-10 mb-6">
              <div
                className="w-32 h-32 rounded-2xl shadow-lg flex items-center justify-center text-white text-4xl font-bold border-4 border-white"
                style={{
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <a href={`tel:${profile.mobno}`} className="font-medium transition" style={{ color: PRIMARY_NAVY }}>
                    {profile.mobno}
                  </a>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {profile.trackingId && (
                <div className="bg-gradient-to-br from-yellow-50 to-amber-100 rounded-2xl shadow-lg p-6 border border-amber-200">
                  ...
                  <p className="text-2xl font-mono font-bold break-all" style={{ color: PRIMARY_NAVY }}>
                    {profile.trackingId}
                  </p>
